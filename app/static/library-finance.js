...
export function financeView(){
  ...
  const draw=()=>{const direction=ui.root.querySelector('#finance-filter').value,q=ui.root.querySelector('#finance-search').value.toLowerCase();ui.root.querySelector('#ledger-rows').innerHTML=f.rows.filter(r=>(!direction||r.direction===direction)&&`${r.title} ${r.owner}`.toLowerCase().includes(q)).map(r=>`<article class="ledger-row"><div class="row between"><strong>${esc(r.title)}</strong><strong class="${r.direction==='수입'?'income':'expense'}">${r.direction==='수입'?'+':'−'}${money(r.amount)}</strong></div><p class="muted">${esc(r.date)} · ${esc(r.classification)} · 담당 ${esc(r.owner)}</p>${r.note?`<p class="prose">${esc(r.note)}</p>`:''}${(r.files||[]).map(f=>`<div class="row"><a class="text-action" href="${fileURL(f)}" target="_blank" rel="noopener">첨부 · ${esc(f.name.replace(/^[0-9a-f-]{36}--/,''))}</a>${canEdit?`<button class="text-action" data-delete-ledger-file="${esc(f.name)}" data-row="${r.id}" data-write>첨부 삭제</button>`:''}</div>`).join('')}${canEdit?`<div class="row"><button class="text-action" data-add-ledger-file="${r.id}" data-write>파일 첨부</button><button class="text-action" data-edit-ledger="${r.id}" data-write>수정</button><button class="text-action" data-delete-ledger="${r.id}" data-write>삭제</button></div>`:''}</article>`).join('')||emptyState('회계 내역이 없어요');ui.root.querySelectorAll('[data-edit-ledger]').forEach(b=>b.onclick=()=>ledgerEditor(f.rows.find(r=>r.id===b.dataset.editLedger)));ui.root.querySelectorAll('[data-add-ledger-file]').forEach(b=>b.onclick=()=>uploadDialog('ledger',b.dataset.addLedgerFile));ui.root.querySelectorAll('[data-delete-ledger-file]').forEach(b=>b.onclick=()=>act('첨부 파일을 삭제할까요?',()=>api('DELETE',`/api/ledger/${b.dataset.row}/files/${encodeURIComponent(b.dataset.deleteLedgerFile)}`)));ui.root.querySelectorAll('[data-delete-ledger]').forEach(b=>b.onclick=()=>act('노션 장부에서 이 항목을 삭제할까요?',()=>api('DELETE',`/api/ledger/${b.dataset.deleteLedger}`)));bindActions();};
