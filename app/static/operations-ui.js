...
function practiceCard(p) {
  ...
  return `<section class="card next-practice"><div class="row between"><p class="eyebrow">${p.category==='지휘'?'다음 연습':'다음 출석 일정'}</p><div class="row">${tag(p)}<a class="text-action" href="#/event/${p.id}">일정 자세히</a></div></div><div class="next-detail">${dateTile(p.starts_at)}<div><h2>${esc(p.title)}</h2><p class="muted">${esc(dateText(p))}<br>${esc(p.place)}</p></div></div>${self}${summary}</section>`;
}
...
export function operationsHome() {
  ...
  const next=events[0], stats=s.stats, practice=events.find(e=>s.my_attendance[e.id]||s.boards?.[e.id]), dup=practice&&next&&next.id===practice.id;
...
export function eventView(id) {
  ...
  ui.root.innerHTML=`${back('#/schedule','캘린더')}<section class="page-heading"><div class="row">${tag(e)}${danger(e)}</div><h1>${esc(e.title)}</h1><p class="muted">${esc(dateText(e))}<br>${esc(e.place)}</p></section>${e.description?`<section class="card prose">${esc(e.description)}</section>`:''}
    <div class="row">${s.my_attendance[id]?`<a class="btn primary" href="#/practice/${id}">내 출석</a>`:''}${s.boards?.[id]?`<a class="btn" href="#/board/${id}">출석 현황</a>`:''}
...
// ---------- 건의함: 누구나 이름을 남겨 보내고, 집행부(단장·홍보·총무)와 파트장·지휘자가 모두 읽고 처리 상태를 바꾼다 ----------
const SUGGESTION_STATUSES=['접수','확인함','반영함','보류'];
const SUGGESTION_CHIP={'접수':'','확인함':'late','반영함':'present','보류':'absent'};
const handles=()=>['head','publicity','treasurer'].includes(ui.state.me.admin_role)||['part_leader','conductor'].includes(ui.state.me.role); // 서버 handles_suggestions 와 같다
...
export function suggestionsView(){
  const s=ui.state, all=handles(), items=s.suggestions||[];
  if(!s.suggestions_enabled){ui.root.innerHTML=emptyState('건의함을 준비하지 못했어요','앱을 만든 사람에게 알려 주세요.');return;}
  ui.root.innerHTML=`<section class="page-heading"><p class="eyebrow">SUGGESTIONS</p><h1>건의함</h1><p class="muted">글리에 바라는 점을 남겨 주세요. 이름과 함께 집행부(단장·홍보·총무)와 파트장·지휘자에게 전달되고, 처리 상태는 아래에서 볼 수 있어요.</p></section>
  ...
  const draw=()=>{
    ...
    ui.root.querySelector('#suggest-list').innerHTML=shown.length?shown.map(x=>{const st=x.status||'접수';return `<article class="ledger-row"><div class="row between">${all?`<strong>${esc(x.member_name)}${x.part?` <span class="muted">${esc(x.part)}</span>`:''}</strong>`:`<span class="muted">${stamp(x.created_at)}</span>`}<span class="chip ${SUGGESTION_CHIP[st]||''}">${esc(st)}</span></div><p class="prose">${esc(x.body)}</p>${all?`<p class="muted">${stamp(x.created_at)}</p>`:''}${x.handled_by?`<p class="muted">처리 ${esc(x.handled_by)} · ${stamp(x.handled_at)}</p>`:''}${all?`<div class="segment suggestion-status" role="group" aria-label="처리 상태">${SUGGESTION_STATUSES.map(v=>`<button type="button" data-mark="${x.id}" data-status="${v}" aria-pressed="${st===v}" data-write>${v}</button>`).join('')}</div>`:''}</article>`;}).join(''):emptyState(all?(items.length?'이 상태의 건의가 없어요':'아직 건의가 없어요'):'보낸 건의가 없어요');
