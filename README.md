...
노션에 일정, 출석, 사진, 곡, 파일, 확인 시각, 학기, 회계를 저장합니다. SQLite는 DB/보기 ID 캐시입니다. 앱을 열거나 저장한 직후에는 노션 기준의 새 스냅숏을 기다립니다. 화면만 옮길 때는 직전 스냅숏으로 바로 그리고, 새 스냅숏은 뒤에서 받아 내용이 달라졌을 때만 다시 그립니다. 조회 실패 시 마지막 내용에 최신 정보가 아님을 표시하고 편집을 막습니다.
