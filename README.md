...
- 건의: 모든 단원이 이름을 남겨 건의를 보내고, 자기가 보낸 건의의 처리 상태를 봅니다. 집행부(단장·홍보·총무)와 파트장·지휘자는 모든 건의를 이름과 함께 보고 접수·확인함·반영함·보류로 처리 상태를 바꿉니다(처리자·시각 기록). 노션 '건의함' DB에 저장되며, 첫 실행 때 부모 페이지(`지휘부 / 출석`) 아래에 자동으로 만들어집니다. 노션에서 그 페이지를 볼 수 있는 파트장·지휘자는 건의를 읽을 수 있고, 일반 단원은 볼 수 없습니다(단체 결정). 준비에 실패하면 건의 메뉴만 숨고 나머지 기능은 그대로 동작합니다. 다른 곳의 DB를 쓰려면 `NOTION_SUGGESTIONS_DATABASE_ID`로 지정하세요(빈 DB면 앱이 열을 채우고 제목 열 이름만 '건의'로 바꿉니다).
