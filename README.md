...
# 건의함은 자동 생성하지 않는다. 집행부만 보는 노션 페이지에 빈 DB를 만들고 통합을 연결한 뒤 지정 (아래 표)
...
앱이 쓰는 노션 DB 9개는 모두 환경변수로 ID를 지정할 수 있습니다. **지정된 DB는 노션 어디에 있어도 됩니다** — 해당 DB(또는 상위 페이지)에 integration이 연결돼 있기만 하면 됩니다. 지정하지 않은 DB는 재배포마다 `NOTION_PARENT_PAGE_ID` 페이지의 **직속 자식에서 제목으로** 다시 찾고, 없으면 새로 만듭니다. Render는 재배포 때 SQLite 캐시가 사라지므로 **전부 지정하는 것을 권장**합니다 — 그래야 DB를 옮기거나 제목을 바꿔도 빈 DB가 중복 생성되지 않습니다.
...
| `NOTION_SUGGESTIONS_DATABASE_ID` | 건의함 (집행부 전용 페이지) | 직접 만든 빈 DB — 지정 전에는 건의 메뉴가 숨는다 |
...
- 재정: 모두 조회하고 총무만 편집합니다. 총무는 회계 항목마다 증빙 파일(형식 제한 없음, 항목당 20개)을 첨부·삭제하고 — 항목을 추가·수정하는 창에서 바로 고를 수도 있습니다 — 첨부는 장부를 보는 모든 단원이 열 수 있습니다. 노션 장부에는 '증빙 파일' 칸으로 저장되며 칸이 없으면 첫 요청 때 자동으로 추가합니다(기존 열·수식은 건드리지 않습니다). 노션 파일 업로드 API가 받지 않는 형식(HWP·EXE·확장자 없는 파일 등)은 서버가 원본을 zip으로 묶어 `이름.확장자.zip`으로 저장합니다. 학기별 수입·지출·이월금·잔액과 분류별 지출을 표시합니다.
- 건의: 모든 단원이 이름을 남겨 건의를 보내고, 자기가 보낸 건의의 처리 상태를 봅니다. 집행부(단장·홍보·총무)는 모든 건의를 보고 접수·확인함·반영함·보류로 처리 상태를 바꿉니다(처리자·시각 기록). 노션 '건의함' DB에 저장됩니다. 이름이 남는 글이라 앱이 자동으로 만들지 않습니다. 집행부만 볼 수 있는 노션 페이지에 빈 DB를 만들고 통합을 연결해 `NOTION_SUGGESTIONS_DATABASE_ID`로 지정하면, 앱이 필요한 열을 채우고(제목 열은 '건의'로 이름만 바꿈) 메뉴를 보여 줍니다.
...
- 환경변수: `NOTION_TOKEN`, `NOTION_PARENT_PAGE_ID`, `SECRET_KEY`, 그리고 "노션 데이터베이스 ID 고정" 표의 `NOTION_*_DATABASE_ID` 9개(권장). 선택적으로 `DB_PATH`, `MAX_UPLOAD_MB`, `NOTION_CACHE_SECONDS`(기본 60초), `PORT`
