...
- 재정: 모두 조회하고 총무만 편집합니다. 총무는 회계 항목마다 증빙 파일(영수증·이체 확인증, PDF/JPG/PNG/HEIC, 항목당 20개)을 첨부·삭제하고, 첨부는 장부를 보는 모든 단원이 열 수 있습니다. 노션 장부에는 '증빙 파일' 칸으로 저장되며 칸이 없으면 첫 요청 때 자동으로 추가합니다(기존 열·수식은 건드리지 않습니다). 학기별 수입·지출·이월금·잔액과 분류별 지출을 표시합니다.
