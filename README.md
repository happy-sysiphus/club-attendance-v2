...
- 출석: 지휘·행정 구분 없이 오늘부터의 전체 일정을 가까운 날짜순으로 표시합니다. 연습·공연과 출석 받기를 켠 행정 일정에서 기존 출석 규칙을 사용합니다.
...
- 행정: 단장·홍보가 모든 일정 사진을 관리합니다. 기한이 지난 사진 미등록 일정에 느낌표와 탭 배지를 표시합니다. 행정 일정 등록·수정·취소·삭제는 단장·홍보가 합니다. 일정을 취소·삭제하면 그 일정의 출석 기록도 지워지므로(노션 휴지통으로 이동), 기록이 있는 일정은 몇 건이 사라지는지 경고한 뒤 진행합니다.
...
단원 본인 출석은 연습이 시작된 뒤에도 입력할 수 있고, 자기 파트가 확인 완료되면 잠깁니다. 그 뒤 수정은 파트장·지휘자가 현황판에서 하며, 수정하면 그 파트의 확인이 풀립니다.

지휘 일정은 항상 출석 대상입니다. 행정 일정은 단장·홍보가 등록·수정할 때 '출석 받기'를 켜야 출석 대상이 되며(노션 '전체 일정'의 '출석 받기' 칸), 이후 흐름은 연습과 같습니다. 출석 기록이 생긴 행정 일정은 출석 받기를 끌 수 없습니다. 노션에서 유형만 '행정'으로 두고 분류가 '지휘'인 일정은 지휘 일정으로 취급해 계속 출석 대상입니다.

## 저장과 파일

노션에 일정, 출석, 사진, 곡, 파일, 확인 시각, 학기, 회계를 저장합니다. SQLite는 DB/보기 ID 캐시입니다. 앱을 열거나 저장한 직후에는 노션 기준의 새 스냅숏을 기다립니다. 화면만 옮길 때는 직전 스냅숏으로 바로 그리고, 새 스냅숏은 뒤에서 받아 내용이 달라졌을 때만 다시 그립니다. 누군가 앱에서 저장하면 서버가 열려 있는 모든 화면에 알리고(`/api/changes`, SSE), 화면은 1~2초 안에 스냅숏을 다시 받아 달라진 내용을 반영합니다. 입력 중인 화면은 입력이 끝난 뒤에 갈아 끼웁니다. 노션에서 직접 고친 값은 알림이 없어 캐시(기본 60초)가 지난 뒤 다음 조회 때 보입니다. 조회 실패 시 마지막 내용에 최신 정보가 아님을 표시하고 편집을 막습니다.

사진 JPG/PNG/HEIC, 필기본 PDF/이미지/MuseScore(MSCZ), 음원 MP3/M4A/WAV를 업로드합니다. 여러 파일을 한 번에 고를 수 있고, 곡 자료는 확장자로 필기본·음원을 나누며 파일마다 '확인 필수'를 따로 정합니다. 노션 파일 업로드 API가 `.mscz` 확장자를 거절하므로 MSCZ는 노션에 `이름.mscz.zip`으로 저장하고(내용은 그대로), 앱에서 열 때 서버가 원래 이름으로 내려줍니다. 기본 서버 제한은 파일당 200MB이며 `MAX_UPLOAD_MB`로 조정할 수 있습니다. 노션 workspace 요금제의 파일 제한도 적용됩니다. HEIC 인라인 표시는 브라우저에 따라 다를 수 있어 원본 열기 링크를 제공합니다.
...
- 단일 프로세스/worker로 실행합니다. 복수 Notion 페이지 갱신은 프로세스 내 잠금으로 직렬화합니다. 실시간 알림도 프로세스 메모리로 전달하므로 worker를 늘리면 다른 worker의 화면에는 알림이 가지 않습니다.
