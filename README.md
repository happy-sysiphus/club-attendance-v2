...
# 건의함은 첫 실행 때 부모 페이지 아래에 자동 생성된다. 생성 뒤 ID를 고정 (아래 표)
...
- 건의: 모든 단원이 이름을 남겨 건의를 보내고, 자기가 보낸 건의의 처리 상태를 봅니다. 집행부(단장·홍보·총무)는 모든 건의를 보고 접수·확인함·반영함·보류로 처리 상태를 바꿉니다(처리자·시각 기록). 노션 '건의함' DB에 저장됩니다.
